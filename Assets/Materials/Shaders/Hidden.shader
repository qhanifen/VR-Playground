...
#define STEP_SIZE 0.01
		...
		{
			float dis = distance(p, _VolumePosition);
			if (dis <= _VolumeRadius + 0.01 && dis >= _VolumeRadius - 0.01)
			{
				return true;
			}
			return false;
